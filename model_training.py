...
def train_model(config_path="./config.yml"):
    ...
    print("Getting data")
    df = pd.read_csv("./DF_ML_Barra_str.csv")
    print(df)
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.2, random_state=7775
    )
    ...
    X_train_transformed = pd.DataFrame(
        X_train, columns=numerical_features 
    )
    X_test_transformed = pd.DataFrame(
        X_test, columns=numerical_features
    )
    print('E')
    print(X_train_transformed)
    ...
    if config["model_training"]["use_grid_search"]:
        ...
        grid_search = GridSearchCV(
            CatBoostRegressor(
                cat_features= numerical_features, eval_metric="MAPE", verbose=False
            ),
